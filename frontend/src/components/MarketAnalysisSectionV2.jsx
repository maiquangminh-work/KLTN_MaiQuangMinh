function MarketAnalysisSectionV2({
  loadingContext,
  contextError,
  marketContext,
  analysisSignalLabel,
  ...
  const alertItems = [
    {
      ...
      note: 'Mức nền để đọc rủi ro thị trường trong ngày.',
    },
    {
      label: 'Tâm lý tin tức',
      value: marketContext?.news_sentiment_label || 'Đang đồng bộ',
      color: marketContext ? getPositiveScoreColor(marketContext.news_sentiment_score) : '#eaecef',
      note: 'Sắc thái chung của luồng tin gần nhất.',
    },
    {
      label: 'Xung lực ngân hàng',
      value: marketContext?.banking_sector_label || 'Đang đồng bộ',
      color: marketContext ? getPositiveScoreColor(marketContext.banking_sector_score) : '#eaecef',
      note: 'Theo dõi trạng thái riêng của nhóm ngân hàng.',
    },
    {
      label: 'Rủi ro chính trị',
      value: marketContext?.political_risk_label || 'Đang đồng bộ',
      color: marketContext ? getRiskScoreColor(marketContext.political_risk_score) : '#eaecef',
      note: 'Yếu tố ngoại sinh có thể làm giá biến động mạnh.',
    },
  ];
      ...
      <div className="detail-grid">
        ...
        <div className="card" style={{ padding: '16px 18px' }}>
          ...
          {!loadingContext && marketContext && (
            <div style={{ marginTop: '12px', background: '#161a1e', border: '1px solid #2b3139', borderRadius: '10px', padding: '12px' }}>
              <div style={{ color: '#848e9c', fontSize: '12px', marginBottom: '6px' }}>Kết luận đọc nhanh</div>
              <div style={{ color: '#cbd5e1', fontSize: '13px', lineHeight: 1.7 }}>
                Ưu tiên đọc cùng lúc áp lực tổng thể, tâm lý tin tức và xung lực ngân hàng trước khi hành động trong ngày.
              </div>
            </div>
          )}
        </div>
...
export default MarketAnalysisSectionV2;
